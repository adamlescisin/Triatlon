...
async function main() {
  ...
  const season = await prisma.season.upsert({
    ...
    update: {},
    create: {
      id: 'season-2024', year: 2024, name: 'Hostín Triatlon 2024',
      date: new Date('2024-07-20'), location: 'Hostín u Vojkovic', status: 'live',
      checkpoints: { create: [
  ...
  const catMuzi = await prisma.category.upsert({
    where: { id: 'cat-muzi' }, update: {},
    create: { id: 'cat-muzi', seasonId: season.id, name: 'Muži 18–39', color: '#3b82f6', type: 'individual', startOrder: 1, startTime: new Date('2024-07-20T09:00:00') },
  })
  const catZeny = await prisma.category.upsert({
    where: { id: 'cat-zeny' }, update: {},
    create: { id: 'cat-zeny', seasonId: season.id, name: 'Ženy', color: '#ec4899', type: 'individual', startOrder: 2, startTime: new Date('2024-07-20T09:05:00') },
  })
  const catStafety = await prisma.category.upsert({
    where: { id: 'cat-stafety' }, update: {},
    create: { id: 'cat-stafety', seasonId: season.id, name: 'Štafety', color: '#8b5cf6', type: 'relay', startOrder: 3, startTime: new Date('2024-07-20T09:10:00') },
  })
  ...
  for (const a of athleteData) {
    const athlete = await prisma.athlete.upsert({ where: { id: a.id }, update: {}, create: { id: a.id, firstName: a.firstName, lastName: a.lastName } })
    await prisma.entry.upsert({
      where: { seasonId_bibNumber: { seasonId: season.id, bibNumber: a.bib } }, update: {},
      create: { seasonId: season.id, categoryId: a.catId, athleteId: athlete.id, bibNumber: a.bib, type: 'individual', status: 'registered' },
    })
  }

  const tkTeam = await prisma.timekeeperTeam.upsert({ where: { id: 'team-1' }, update: {}, create: { id: 'team-1', categoryId: catMuzi.id, name: 'Tým A' } })
  await prisma.timekeeperTeamMember.upsert({ where: { id: 'tkm-1' }, update: {}, create: { id: 'tkm-1', timekeeperTeamId: tkTeam.id, userId: timekeeper1.id } })

  console.log('Seed complete! admin/admin123  timekeeper1/timer123  announcer1/announce123')
}

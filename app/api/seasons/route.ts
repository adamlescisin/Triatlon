...
export async function GET() {
  const seasons = await prisma.season.findMany({ include: { categories: true, checkpoints: { orderBy: { order: 'asc' } } }, orderBy: { year: 'desc' } })
...
export async function POST(req: NextRequest) {
  ...
  const season = await prisma.season.create({
    data: {
      year: body.year, name: body.name, date: new Date(body.date), location: body.location,
      checkpoints: { create: [

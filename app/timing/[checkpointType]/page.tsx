...
export default function TimingCheckpointPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/entries?seasonId=${seasonId}`)
      .then(r => r.json())
      .then((data: any[]) => {
        setEntries(data)
        const alreadyRecorded = new Set<string>()
        data.forEach(e => {
          if (e.timeRecords?.some((r: any) => r.checkpointId === checkpointId)) {
            alreadyRecorded.add(e.id)
          }
        })
  ...
  const recordTime = useCallback(async (entry: any) => {
    ...
    const name = entry.athlete
      ...
      : entry.relayTeam?.name ?? `#${entry.bibNumber}`
    ...
    if (online) {
      try {
        await fetch('/api/time-records', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ entryId: entry.id, checkpointId, timestamp }),
        })
  ...
  const undoLast = async () => {
    ...
    setRecorded(prev => {
      ...
      next.delete(lastAction.entryId)
      return next
    })
  ...
  const cancelPress = (entryId: string) => {
    if (progressInterval.current[entryId]) {
    ...
    <div className="min-h-screen bg-gray-100 pb-20">
      <header className="bg-emerald-800 text-white px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex-1">
          <h1 className="font-bold leading-tight">Checkpoint</h1>
          <p className="text-emerald-300 text-xs">{session?.user?.name}</p>
        </div>
        <div className="flex items-center gap-2">
          {pendingCount > 0 && (
            <span className="bg-orange-500 text-white text-xs px-2 py-0.5 rounded-full">{pendingCount} čeká</span>
          )}
      ...
      {lastAction && (
        <div className="bg-emerald-700 text-white px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={undoLast} className="flex items-center gap-1 bg-white/20 hover:bg-white/30 px-3 py-1 rounded-lg text-sm">
            <RotateCcw size={14} /> Vrátit
          </button>
        </div>
      )}
      ...
      <div className="p-3">
        {activeEntries.length > 0 && (
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wide font-medium mb-2 px-1">
              Ještě nezaznamenáno ({activeEntries.length})
            </p>
            <div className="grid grid-cols-3 gap-2 mb-4">
              {activeEntries.map((entry: any) => {
                const progress = pressing[entry.id] ?? 0
                const isPressed = progress > 0
                const name = entry.athlete
                  ? `${entry.athlete.firstName[0]}. ${entry.athlete.lastName}`
                ...
                return (
                  <button
                    ...
                    onPointerUp={() => cancelPress(entry.id)}
                    onPointerLeave={() => cancelPress(entry.id)}
                    onContextMenu={e => e.preventDefault()}
                    className="relative select-none touch-none overflow-hidden bg-white rounded-xl shadow-md p-3 flex flex-col items-center gap-1 active:shadow-inner transition-all"
                    style={{ userSelect: 'none', WebkitUserSelect: 'none' }}
                  >
                    {isPressed && (
                      <div
                        className="absolute inset-0 bg-orange-400/30 transition-none"
                        style={{ width: `${progress}%` }}
                      />
                    )}
                    <span
                      className="text-2xl font-black leading-none z-10"
                      style={{ color: entry.category?.color ?? '#065f46' }}
                    >
                      {entry.bibNumber}
                    </span>
                    <span className="text-xs text-gray-600 text-center leading-tight z-10 line-clamp-1">{name}</span>
                    <span
                      className="text-xs px-1.5 py-0.5 rounded-full z-10 text-white text-center"
                      style={{ backgroundColor: entry.category?.color ?? '#065f46' }}
                    >
                      {entry.category?.name?.split(' ')[0]}
                    </span>
                  </button>
                )
              })}
        ...
        {doneEntries.length > 0 && (
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wide font-medium mb-2 px-1">
              Zaznamenáno ({doneEntries.length})
            </p>
            <div className="grid grid-cols-4 gap-2">
              {doneEntries.map((entry: any) => (
                <div
                  key={entry.id}
                  className="bg-gray-200 rounded-xl p-2 flex flex-col items-center gap-0.5 opacity-60"
                >
                  <Check size={12} className="text-emerald-700" />
                  <span className="text-lg font-black text-gray-500">{entry.bibNumber}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {entries.length === 0 && (
          <div className="text-center py-16 text-gray-400">
            <p className="text-lg font-medium">Žádní závodníci</p>
            <p className="text-sm">Zkontrolujte přiřazení sezóny</p>
          </div>
        )}
      </div>

...
export default function AnnouncerPage() {
  ...
  const sendAlert = async () => {
    ...
    const message = customMessage || (entry
      ? `Závodník č. ${entry.bibNumber}, ${athleteName} (${entry.category?.name}), se blíží — ${cpLabel}`
      : 'Oznámení')

    await fetch('/api/announcer-alerts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ entryId: entry?.id, checkpointType: checkpoint, message }),
    })
    setSending(false)
    setSelectedEntry('')
    setCustomMessage('')
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <header className="bg-emerald-800 text-white px-4 py-4 flex items-center gap-3">
        <Mic size={24} />
        <div>
          <h1 className="font-bold text-lg">Hlasatel</h1>
      ...
      <div className="p-4 space-y-4 max-w-2xl mx-auto">
        <div className="bg-white rounded-xl shadow p-4">
          ...
          <div className="mb-3">
            ...
            <input
              ...
              className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 mb-2"
            />
            <div className="max-h-40 overflow-y-auto border rounded-lg divide-y">
              <button
                onClick={() => { setSelectedEntry(''); setSearch('') }}
                className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${!selectedEntry ? 'bg-emerald-50 text-emerald-700 font-medium' : ''}`}
              >
                Žádný (vlastní zpráva)
              </button>
              {filtered.slice(0, 20).map(e => {
                ...
                return (
                  <button
                    key={e.id}
                    onClick={() => { setSelectedEntry(e.id); setSearch('') }}
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 ${selectedEntry === e.id ? 'bg-emerald-50 text-emerald-700 font-medium' : ''}`}
                  >
                    <span className="font-bold w-8 text-center" style={{ color: e.category?.color }}>
                      #{e.bibNumber}
                    </span>
                    <span>{name}</span>
                    <span className="text-xs text-gray-400 ml-auto">{e.category?.name}</span>
                  </button>
                )
              })}
            </div>
          ...
          {selectedEntryObj && (
            <div className="bg-emerald-50 rounded-lg p-3 mb-3 text-sm">
              <p className="font-medium text-emerald-800">Náhled oznámení:</p>
              <p className="text-emerald-700 mt-1 italic">
                „Závodník č. {selectedEntryObj.bibNumber},{' '}
                {selectedEntryObj.athlete
                  ? `${selectedEntryObj.athlete.firstName} ${selectedEntryObj.athlete.lastName}`
                  : selectedEntryObj.relayTeam?.name}{' '}
                ({selectedEntryObj.category?.name}), se blíží —{' '}
                {CHECKPOINT_OPTIONS.find(c => c.value === checkpoint)?.label}“
              </p>
            </div>
          )}
          ...
          {!selectedEntryObj && (
            <div className="mb-3">
              <label className="block text-sm font-medium text-gray-700 mb-1">Vlastní zpráva</label>
              <textarea
                value={customMessage}
          ...
          <button
            onClick={sendAlert}
            disabled={sending || (!selectedEntry && !customMessage)}
            className="w-full flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 disabled:bg-gray-300 text-white py-3 rounded-xl font-bold transition-colors"
          >
            <Send size={18} />
        ...
        <div className="bg-white rounded-xl shadow p-4">
          ...
          {alerts.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-4">Žádná oznámení</p>
          ) : (
              ...
              {alerts.map((alert: any) => (
                <div key={alert.id} className="border rounded-lg p-3">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {new Date(alert.createdAt).toLocaleTimeString('cs-CZ')} · {alert.author?.name}
                  </p>
                </div>
              ))}
            </div>

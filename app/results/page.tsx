          ...
          <div>
            ...
            {season?.status === 'live' && (
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-orange-300 text-sm font-medium">ŽIVĚ · aktualizuje se automaticky</span>
              </div>
            )}
          </div>
      ...
      {alerts.length > 0 && (
        <div className="bg-orange-600 text-white px-4 py-2">
          <div className="max-w-4xl mx-auto">
            <p className="text-xs font-bold uppercase tracking-wide mb-1">📢 Hlasatel</p>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        {!season && (
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-lg font-medium">Žádné výsledky k zobrazení</p>
          </div>
        )}
            ...
            <div className="divide-y">
              {cat.entries.map((entry: any, idx: number) => {
                ...
                const name = entry.athlete
                  ...
                  : entry.relayTeam?.name ?? `Tým #${entry.bibNumber}`

                return (
                  <div key={entry.id} className="px-4 py-3 flex items-center gap-3">
                    <span className="text-xl font-black w-8 text-center text-gray-300">
                      {entry.computed?.rank ?? (idx + 1)}
                    </span>
                    <span className="font-bold text-lg w-10 text-center" style={{ color: cat.color }}>
                      {entry.bibNumber}
                    </span>
                    <div className="flex-1 min-w-0">
                      <Link href={`/results/${entry.athleteId ?? 'relay'}`} className="font-medium text-gray-900 hover:text-emerald-700 truncate block">
                        {name}
                      </Link>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${status.color}`}>{status.label}</span>
                    </div>
                    <div className="text-right flex-shrink-0">
                      {entry.computed?.totalTime ? (
                        <p className="font-mono font-bold text-gray-900 text-sm">{formatMs(entry.computed.totalTime)}</p>
                      ) : (
              ...
              {cat.entries.length === 0 && (
                <div className="px-4 py-6 text-center text-gray-400 text-sm">Žádní závodníci</div>
              )}
            </div>
          </div>
        ))}

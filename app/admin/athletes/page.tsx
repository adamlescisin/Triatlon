  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Závodníci</h1>
          <p className="text-gray-500 mt-1">Správa startovního pole</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow p-6 mb-4">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nouzový kontakt (neveřejný)</label>
          ...
          <div className="flex gap-2">
            <button onClick={save} disabled={saving || !form.firstName || !form.lastName}
              className="bg-emerald-800 hover:bg-emerald-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium">
              {saving ? 'Ukládám...' : 'Uložit'}
            </button>
            <button onClick={() => setShowForm(false)} className="text-gray-600 px-4 py-2 rounded-lg text-sm hover:bg-gray-100">
              Zrušit
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-400">Žádní závodníci</p>
          </div>
        ) : (
              ...
              {athletes.map((a: any) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-600">{age ?? '—'}</td>
                    <td className="px-6 py-4 text-gray-600">{a.entries?.length ?? 0}</td>
                    <td className="px-6 py-4">
                      <Link href={`/admin/athletes/${a.id}`} className="text-emerald-600 hover:text-emerald-800 font-medium text-sm">
                        Detail
                      </Link>
                    </td>
                  </tr>

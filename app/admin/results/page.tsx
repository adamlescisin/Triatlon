...
export default async function ResultsAuditPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Záznamy časů</h1>
        <p className="text-gray-500 mt-1">Audit log — posledních 100 záznamů</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        {records.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-400">Žádné záznamy</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="text-left px-4 py-3 text-gray-500 font-medium">Čas</th>
                ...
                <tr key={r.id} className={`hover:bg-gray-50 ${r.isCorrection ? 'bg-yellow-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full" style={{ backgroundColor: r.entry?.category?.color }} />
                      {r.entry?.athlete ? `${r.entry.athlete.firstName} ${r.entry.athlete.lastName}` : r.entry?.id?.slice(0, 8)}
                    </div>
                  </td>

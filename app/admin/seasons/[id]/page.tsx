...
export default function SeasonDetailPage() {
  ...
  const addCategory = async () => {
    ...
    const res = await fetch('/api/categories', {
      ...
      body: JSON.stringify({ ...newCategory, seasonId: id }),
    })
    const cat = await res.json()
    setSeason((s: any) => ({ ...s, categories: [...s.categories, { ...cat, entries: [], timekeeperTeams: [] }] }))
    ...
    <div className="p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500">{season.location} · {season.year}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 mb-1">Stav závodu</p>
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${statusColors[season.status]}`}>
              {statusLabels[season.status]}
            </span>
          </div>
          <div className="flex gap-2">
            {season.status === 'preparing' && (
              <button onClick={() => updateStatus('live')} className="bg-orange-600 hover:bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium">
                Spustit závod (LIVE)
              </button>
            )}
            {season.status === 'live' && (
              <button onClick={() => updateStatus('closed')} className="bg-gray-600 hover:bg-gray-500 text-white px-4 py-2 rounded-lg text-sm font-medium">
                Uzavřít závod
              </button>
            )}
          </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {season.checkpoints.map((cp: any) => (
            <span key={cp.id} className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm">
              {cp.order}. {cp.label}
            </span>
          ))}
        </div>
      ...
      <div className="bg-white rounded-xl shadow p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setShowCategoryForm(true)} className="flex items-center gap-2 bg-emerald-800 hover:bg-emerald-700 text-white px-3 py-2 rounded-lg text-sm">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div>
                ...
                <select value={newCategory.type} onChange={e => setNewCategory(p => ({ ...p, type: e.target.value }))}
                  ...
                  <option value="relay">Štafety</option>
                </select>
              </div>
            </div>
            <div className="mb-3">
              ...
              <div className="flex gap-2">
                {COLORS.map(c => (
                  <button key={c} onClick={() => setNewCategory(p => ({ ...p, color: c }))}
                    className={`w-8 h-8 rounded-full border-2 ${newCategory.color === c ? 'border-gray-900 scale-110' : 'border-transparent'}`}
                    style={{ backgroundColor: c }} />
                ))}
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={addCategory} disabled={saving || !newCategory.name}
                className="flex items-center gap-1 bg-emerald-800 text-white px-3 py-2 rounded-lg text-sm disabled:opacity-50">
                <Check size={14} /> Uložit
              </button>
              <button onClick={() => setShowCategoryForm(false)} className="flex items-center gap-1 text-gray-600 px-3 py-2 rounded-lg text-sm hover:bg-gray-100">
                <X size={14} /> Zrušit
              </button>
            </div>
          </div>
